...
class base_api {
  ...
  //  let base_url = "http://192.168.17.1"
    ...
    func search_request(type : String , latitude : Double  , longitude : Double ,  searchname : String , distance : Int) -> URLRequest{
        var url : URL
        var jsondata : [String : Any]
        switch type {
        case base_api().search_by_category():
            jsondata = [ "latitude": latitude , "longitude" : longitude , "categoryName" : searchname , "distance" : distance ]
            url = URL(string: "\(base_url)/Shops/shops/categories/search_by_category")!
            break
        case base_api().search_by_intelligentsearch():
            jsondata = [ "latitude": latitude , "longitude" : longitude ,"description": searchname , "distance" : distance]
            url = URL(string: "\(base_url)/Shops/shops/products/search_by_description")!
            break
        case base_api().search_by_product():
            jsondata = ["latitude": latitude , "longitude" : longitude , "productName":searchname , "distance" : distance ]
